...
class ExploreGridTaus:
    ...
    :param denominator: an integer or a list of integers. The grain of the grid. For example, if ``denominator=9``,
        we will examine tau-vectors such as (1/9, 2/9, 2/9, 1/9, 1/3, 0) or (1/9, 5/9, 0, 3/9, 0, 0), etc. If a list of
        integers is given, then all the corresponding denominators will be examined.
    :param test: a function ``TauVector -> bool``. Only the tau-vectors satisfying this test will be examined.
        Default: always True.
    :param standardized: if True, then we examine only the standardized tau-vectors (i.e. up to relabelling the
        candidates).
