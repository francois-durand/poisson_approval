...
class StrategyThreshold(StrategyTwelve):
    """A threshold strategy (for a continuous profile).

    Parameters
    ----------
    d_ranking_threshold : dict
        Keys are rankings and values are utility thresholds. E.g. ``'abc': 0.4`` means that a voter ``abc`` vote for
        ``ab`` if her utility for ``b`` is strictly greater than 0.4, and for ``a`` if her utility for ``b`` is
        strictly lower than 0.4. The strategy does not specify what the voter does if her utility for ``b`` is
        equal to 0.4: that is why it is only adapted for continuous profiles. A threshold can be ``None``, meaning that
        the behavior of these voters is not specified in the strategy.
